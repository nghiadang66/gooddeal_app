...
import { createTransactionByUser } from '../../services/transaction';
...
const UserWithdraw = ({ navigation, route }) => {
    const [isloading, setIsLoading] = useState(false);
    const [error, setError] = useState('');
    const [success, setSuccess] = useState('');

    const { jwt, setUserProfile, userProfile } = useContext(AuthContext);
    ...
    const handleValidate = (isValidName, flag) => {
        if (isValidName === 'isValidAmount') {
            setTransaction({
                ...
                isValidAmount:
                    flag &&
                    parseFloat(transaction.amount) <= parseFloat(userProfile.e_wallet.$numberDecimal),
            });
        } else
            setTransaction({
                ...transaction,
    ...
    const handleSubmit = () => {
        ...
        if (parseFloat(amount) > parseFloat(userProfile.e_wallet.$numberDecimal)) {
            setTransaction({
                ...transaction,
                isValidAmount:
                    numberTest('positive', amount) &&
    ...
    const onSubmit = () => {
        ...
        setSuccess('');
        setIsLoading(true);
        createTransactionByUser(jwt._id, jwt.accessToken, transaction)
            .then(data => {
                if (data.error) {
                    setError(data.error);
                    setTimeout(() => {
            ...
            <View style={[styles.rowContainer, styles.m6]}>
                ...
                <View style={styles.container}>
                    <Text style={styles.wallet}>
                        {userProfile.e_wallet ? 
                            formatPrice(userProfile.e_wallet.$numberDecimal) : 
                            0
                        }
                    </Text>
                </View>
